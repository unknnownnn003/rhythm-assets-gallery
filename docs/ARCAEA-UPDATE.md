...
```
准备新旧两版 APK → 提取新增图片 → 更新元信息 → 重新生成站点
```
...
| `世界模式/` | 世界模式相关图片 |
| `启动页面/` | 启动页面图片 |
| `_metadata/` | 新版 `songlist`、`packlist`、`story2/ordering` 副本 |
...
- 角色立绘/头像/LinkPlay → 关联 `搭档列表.CSV` 中的搭档名
...
| 构建站点 | `npm run build` |
| 全量更新并部署 | `npm run update && npm run build && .\scripts\deploy.ps1 -Mode remote-build` |
