...
$avatarName = -join ([char]0x5934, [char]0x50CF)
$defaultRemoteGameDir = "Phigros"
$defaultPhigrosRoot = Join-Path (Join-Path "D:\Files" $curveArtName) "Phigros"
...
function Resolve-LocalDir {
  ...
  if (-not [string]::IsNullOrWhiteSpace($InputVersion)) {
    $candidate = Join-Path $defaultPhigrosRoot ($InputVersion.Replace(".", "_"))
    if (-not (Test-Path -LiteralPath $candidate -PathType Container)) {
      throw "Local update directory for version $InputVersion does not exist: $candidate"
    }
    return (Resolve-Path -LiteralPath $candidate).Path
  }

  if (-not (Test-Path -LiteralPath $defaultPhigrosRoot -PathType Container)) {
    throw "Default Phigros directory does not exist: $defaultPhigrosRoot"
  }

  $latest = Get-ChildItem -LiteralPath $defaultPhigrosRoot -Directory |
    Where-Object { $_.Name -match '^\d+(?:_\d+)+$' } |
    Sort-Object { Get-VersionKey $_.Name } -Descending |
    Select-Object -First 1

  if ($null -eq $latest) {
    throw "No version directory found under $defaultPhigrosRoot. Pass -LocalDir explicitly."
  }
...
function Get-UploadDirectories {
  ...
  $names = @($script:curveArtName, $script:avatarName)
  $dirs = @()
  foreach ($name in $names) {
    $path = Join-Path $Root $name
    if (Test-Path -LiteralPath $path -PathType Container) {
      $hasFiles = @(Get-ChildItem -LiteralPath $path -Recurse -File -Include *.png,*.jpg,*.jpeg,*.webp,*.avif,*.gif).Count -gt 0
      if ($hasFiles) {
  ...
  if ($dirs.Count -eq 0) {
    throw "No uploadable image directories found under $Root. Expected one or both of the Phigros image category directories."
  }

  return $dirs
}

function Get-RemotePhigrosDirs {
  param(
    [string[]]$SshArgs,
    [string]$Remote,
  ...
  $remoteScript = "if [ -d $quotedRoot ]; then find $quotedRoot -mindepth 1 -maxdepth 1 -type d -name 'Phigros*' -printf '%f\n' | sort; fi"
  $output = & ssh @SshArgs $Remote $remoteScript
  if ($LASTEXITCODE -ne 0) {
    throw "Failed to list remote Phigros directories."
  }
...
function Select-PreviousRemoteGameDir {
  ...
  foreach ($candidate in $Candidates) {
    ...
    if ($candidate -like "Phigros*" -and $candidate -match '(\d+(?:\.\d+)+)') {
...
if ([string]::IsNullOrWhiteSpace($resolvedVersion)) {
  throw "Could not infer Phigros version from local directory. Pass -Version explicitly."
}

$targetRemoteGameDir = if ([string]::IsNullOrWhiteSpace($RemoteGameDir)) {
  $defaultRemoteGameDir
} else {
  $RemoteGameDir
}
...
$remoteCandidates = Get-RemotePhigrosDirs -SshArgs $sshBaseArgs -Remote $remote -RemoteAssetRoot $remoteAssetRoot
...
Write-Host "publish-phigros-update: local=$resolvedLocalDir"
Write-Host "publish-phigros-update: version=$resolvedVersion"
Write-Host "publish-phigros-update: remoteAssetRoot=$remoteAssetRoot"
Write-Host "publish-phigros-update: target=$targetRemoteDir"
if (-not [string]::IsNullOrWhiteSpace($sourceRemoteGameDir)) {
  Write-Host "publish-phigros-update: previous=$sourceRemoteGameDir"
}

Invoke-Step "Prepare remote Phigros directory" {
...
Invoke-Step "Upload Phigros image directories" {
  $quotedTarget = Quote-RemotePath $targetRemoteDir
...
if (-not $KeepPrevious -and -not [string]::IsNullOrWhiteSpace($sourceRemoteGameDir) -and $sourceRemoteGameDir -ne $targetRemoteGameDir) {
  Invoke-Step "Move previous Phigros directory out of asset root" {
    $sourceRemoteDir = Join-RemotePath -Parent $remoteAssetRoot -Child $sourceRemoteGameDir
    $backupRoot = Join-RemotePath -Parent $remoteWorkPath -Child "asset-backups/phigros"
...
Write-Host ""
Write-Host "publish-phigros-update: done"
